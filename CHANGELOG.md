...
- **Session replay: Canvas/WebGL capture toggle** (`@marlinjai/analytics-tracker` 1.4.0). New tri-state project setting `recordCanvas` ('auto' | 'on' | 'off'), surfaced as a segmented control in dashboard Settings. 'auto' (default) records `<canvas>`/WebGL content only when the page actually has a canvas, so non-canvas pages avoid the much larger canvas payload; 'on' always records, 'off' never. Wired through `GET /api/projects/{id}/config`, `resolveRecordCanvas()` in the tracker, and rrweb's `recordCanvas` option. Only applies when Session Replay is enabled.

### Fixed
- **Session replay: cross-origin images no longer render blank** (`@marlinjai/analytics-tracker` 1.4.0). rrweb's `inlineImages` drew every `<img>` to a canvas and called `toDataURL()`, which taints the canvas for cross-origin images served without CORS and leaves the replay image blank (the reported "blank product image" symptom). The tracker now records the original `<img src>` (`inlineImages: false`): a plain `<img>` renders cross-origin fine on playback without CORS, and replay payloads shrink. Images whose URL is no longer reachable at playback time are handled by the planned server-side asset-rehosting pipeline (`docs/superpowers/plans/2026-06-25-session-replay-asset-rehosting-pipeline.md`), not by inlining.
