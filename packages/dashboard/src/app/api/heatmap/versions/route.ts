...
import { pageVersionQuerySchema } from '@analytics-platform/shared';
import { getPageVersions } from '@/lib/queries/heatmap';
...
export async function GET(request: NextRequest) {
  ...
  const parsed = pageVersionQuerySchema.safeParse({
  ...
  const versions = await getPageVersions(projectId, url);
