...
> [!done] Cutover COMPLETE (2026-06-19)
> Phase 4 (auth-brain cutover) is done and verified live. Decision: **Option A (per-project workspace)** under a dedicated `lumitra-analytics` TENANT (sibling of `lumitra-core`), which the merged runtime gate already requires (`auth-check.ts` resolves `projects.workspace_id` per project). Identity consolidated onto `marlinjaipohl@gmail.com`. Executed: tenant + 2 per-project workspaces + OpenFGA tuples; `projects.workspace_id` backfilled; migration `014` reworked (drops the dead NextAuth tables + the `account_api_keys`->users FK) and applied; the 3 `account_api_keys` (lola-stories/CLI) **remapped** to the auth-brain user id and PRESERVED; analytics prod env set + redeployed. Live: `analytics.lumitra.co/dashboard` 307 -> `auth.lumitra.co/login`; ingestion (`/api/collect`, `/api/ingest`) NOT gated. **No Lola data lost**: ClickHouse `analytics.events` (24,309) is keyed by project_id and untouched; projects + ingestion keys preserved; pre-migration `pg_dump` snapshot retained. Follow-ups: commit the analytics repo changes; re-wire the interactive CLI device-login (`/api/cli-auth/*`) to auth-brain (the env-var account-key path is unaffected). See memory `analytics-auth-brain-cutover-state`.
